#ifndef XENGINE_CORE_COUNTER_H
#define XENGINE_CORE_COUNTER_H
...
#define COUNTER_READ(TAG) \
  ...
  COUNTER_READ_CRUMB(TAG, 128, \
  0 ) ) ) ) ) ) ) )

#define COUNTER_INC(TAG) \
  constant_index<COUNTER_READ(TAG) + 1> \
  constexpr counter_crumb(TAG, constant_index<(COUNTER_READ(TAG) + 1) & ~ COUNTER_READ(TAG)>, constant_index<(COUNTER_READ(TAG) + 1) & COUNTER_READ(TAG)>) { return {}; }
...
counter_crumb(id, constant_index<rank>, constant_index<acc>) { return {}; } // found by ADL via constant_index

#endif // XENGINE_CORE_COUNTER_H